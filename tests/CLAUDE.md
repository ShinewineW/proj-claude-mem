...
**Gotcha**: `mock.module()` is IRREVERSIBLE — `mock.restore()` does NOT clear it. A test importing a module that other files `mock.module()` (ProcessRegistry, ChromaMcpManager, SessionStore, ChromaSync) passes in isolation but FAILS in the full suite. Fix: source-inspection (`readFileSync` the `.ts`), inline a faithful copy (see `process-registry-killed.test.ts`), or convert the *polluting* test to restorable `spyOn`. The FULL `bun test` (stable order) is the gate — a subset run like `bun test tests/sqlite/ tests/worker/` surfaces phantom pollution failures the full run does not.
