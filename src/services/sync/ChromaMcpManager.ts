...
export class ChromaMcpManager {
  ...
  private async connectInternal(): Promise<void> {
    ...
    // The onnxruntime CPU cap is applied via the spawn environment, not the
    // command: getSpawnEnv() injects a sitecustomize shim (PYTHONPATH) that
    // bounds the intra-op thread pool. See ensureOrtCapSitecustomize() for why
    // taskset/cgroup affinity does not work here.
  ...
  /**
   * Write (idempotently) a sitecustomize.py that caps onnxruntime's intra-op
   * thread pool, and return the directory to place on the subprocess PYTHONPATH.
   *
   * Why a sitecustomize shim and not taskset / cgroup / env: the onnxruntime
   * wheel is non-OpenMP (ignores OMP_NUM_THREADS) and pins each intra-op worker
   * thread to a core via pthread_setaffinity_np using its OWN host-core
   * enumeration, so it escapes an inherited taskset/cgroup affinity mask; and a
   * cpuset child cgroup is unavailable when /sys/fs/cgroup is mounted read-only
   * (non-privileged k8s pods). The one lever that actually bounds it is
   * SessionOptions.intra_op_num_threads, which chromadb's embedding function
   * never sets and exposes no knob for. Python auto-imports sitecustomize at
   * interpreter startup — before chromadb builds its session — so monkeypatching
   * InferenceSession there forces the cap regardless of uvx env rebuilds.
   * Verified empirically: caps a default-options session from ~12-16 cores to ~1.
   */
  private ensureOrtCapSitecustomize(): string {
    const dir = path.join(os.homedir(), '.claude-mem', 'runtime', 'ort-cap');
    const body = `# Auto-generated by claude-mem ChromaMcpManager. Do not edit.
# Caps onnxruntime's intra-op thread pool so the CPU embedder can't oversubscribe
# the host core count inside a small cgroup. Size via CLAUDE_MEM_ORT_INTRA_OP_THREADS.
import os
_n = int(os.environ.get("CLAUDE_MEM_ORT_INTRA_OP_THREADS", "0") or "0")
if _n > 0:
    try:
        ...
        _ort.InferenceSession.__init__ = _capped
    except Exception:
        pass
`;
    try {
      fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(path.join(dir, 'sitecustomize.py'), body);
    } catch (err) {
      logger.warn('CHROMA_MCP', 'Failed to write onnxruntime thread-cap shim', {
  ...
  private getSpawnEnv(): Record<string, string> {
    ...
    // Bound the embedding subprocess's CPU on big-host/small-cgroup setups.
    // Primary lever: a sitecustomize shim on PYTHONPATH (see
    // ensureOrtCapSitecustomize) that caps onnxruntime's intra-op pool — the
    // only thing that actually holds, since onnxruntime ignores OMP/affinity.
    // The *_NUM_THREADS vars trim the auxiliary OpenBLAS/numpy/tokenizer pools.
    // Only set when unset by the user. Must precede the combinedCertPath
    // early-return so it applies on every path.
    const chromaThreads = String(this.getChromaCpuLimit());
    if (!baseEnv.CLAUDE_MEM_ORT_INTRA_OP_THREADS) baseEnv.CLAUDE_MEM_ORT_INTRA_OP_THREADS = chromaThreads;
    const sitecustomizeDir = this.ensureOrtCapSitecustomize();
    baseEnv.PYTHONPATH = baseEnv.PYTHONPATH
      ? `${sitecustomizeDir}${path.delimiter}${baseEnv.PYTHONPATH}`
      : sitecustomizeDir;
