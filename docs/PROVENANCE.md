...
| `e8bf1b37` | mark pending messages failed + log preview on non-XML | `92f800d4` | AGPL-3.0 |
| `cdcc7330` | strip `<system-reminder>` and `<persisted-output>` tags | `31ee1024c` + `f81684c61` | AGPL-3.0 |
| `894f187c` | strip privacy tags from last_assistant_message | `a66b98bcd` | AGPL-3.0 |
| `e941395d` | real no-tools lockdown for observer + fresh-summarize SDK | `ce13c887` | Apache-2.0 |
| `bfd99228` | restrict .env + settings.json to owner-only (0600/0700) | `9c56dda79` | Apache-2.0 |
| `5be5308f` | atomic socket-bind isPortInUse to kill TOCTOU spawn race | `46d204ee9` | AGPL-3.0 |
| `9d6e2b80` | exit cleanly when duplicate daemon loses port bind race | `703c64c7` | AGPL-3.0 |
...
| `6e984aa6` | reset memorySessionId + forceInit on context overflow | `08cf2ba3` | AGPL-3.0 |
...
- Chunk 7 Tasks 1–2 已在 P2 第一阶段提交（`8f550102`, `23514f1d`），本文件覆盖全轮所有 38 个 upstream-derived 提交 + 3 个 fork-original。
- 同一上游 hash 在多个 fork 提交中出现（`be99a5d69` × 3, `46d204ee9` × 2, `88b47f9e` × 2, `55334129` × 2, `f97c50bf` × 2）属正常情况 — 同一上游修复被拆分为独立 fork 提交。
- 树形杀进程提交 `d1af69e0` 来自双上游源（`d384d3c5` AGPL-3.0 + `55334129` Apache-2.0），两者均已追溯。
