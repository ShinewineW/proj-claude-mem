...
**Fresh Summarize Path** (`src/services/worker/SummaryLane.ts`, `fresh-summarize.ts`, `fresh-summarize-store.ts`, `fresh-summarize-deps.ts`) - Summaries run on a fresh `query()` subprocess via the **global single-consumer `SummaryLane`**, which claims `type='summarize'` rows from `pending_messages` and calls `runFreshSummarizeQuery` (no resume, no observer priming) to bypass the observer-session role conditioning that empirically produced 0% valid `<summary>` XML in production. The Stop hook is fire-and-forget (~5ms); turn identity is `(content_session_id, turn_number)`, while `prompt_number` is the latest non-redacted prompt used for attribution and may repeat across turns. The atomic store re-reads `memory_session_id` inside a transaction to avoid the FK race. The old observer-response Case 1/2/3 salvage branches are removed; a distinct dead-letter fallback remains: after the configured 3 retries are exhausted and the next failure dead-letters the row, `SummaryLane.salvageAfterDeadLetter()` may persist a prompt-scoped, DB-derived summary explicitly labeled `[salvaged]`. A salvaged row is failure recovery, never evidence that fresh summarize succeeded. Full architecture → `src/services/worker/CLAUDE.md`.
...
bun test ./tests/         # Full fork gate; infrastructure tests reflect local plugin registration state. Scope to ./tests/ — a bare `bun test` also scans the upstream clone in attn_sink/.
```
