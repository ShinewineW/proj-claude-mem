...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:l,memorySessionId:g,correlationId:b,...E}=n;Object.keys(E).length>0&&(u=` {${Object.entries(E).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${p}${s}${u}${m}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},_=new V;var Mt={};function At(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,_e.fileURLToPath)(Mt.url))}var It=At(),A=process.env.CLAUDE_MEM_DATA_DIR||I.get("CLAUDE_MEM_DATA_DIR"),M=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,pe.homedir)(),".claude"),ts=(0,h.join)(M,"plugins","marketplaces","thedotmack"),ss=(0,h.join)(A,"archives"),rs=(0,h.join)(A,"logs"),ns=(0,h.join)(A,"trash"),os=(0,h.join)(A,"backups"),is=(0,h.join)(A,"modes"),as=(0,h.join)(A,"settings.json"),me=(0,h.join)(A,"claude-mem.db"),ds=(0,h.join)(A,"vector-db"),cs=(0,h.join)(A,"observer-sessions"),ps=(0,h.join)(M,"settings.json"),_s=(0,h.join)(M,"commands"),ms=(0,h.join)(M,"CLAUDE.md");function ue(r){(0,F.mkdirSync)(r,{recursive:!0})}function le(){return(0,h.join)(It,"..")}var Ee=require("crypto");var Lt=3e4;function Te(r,e,t){return(0,Ee.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function ge(r,e,t){let s=t-Lt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var D=class{db;constructor(e=me){e!==":memory:"&&ue((0,Se.dirname)(e)),this.db=new fe.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addObservationAccessCountColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,p=s==="date_asc"?"ASC":"DESC",m=n?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),T=[...e],l=[];if(o&&(l.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let E=i.map(()=>"?").join(",");l.push(`type IN (${E})`),T.push(...i)}else l.push("type = ?"),T.push(i);if(a){let E=Array.isArray(a)?a:[a],S=E.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...E),l.push(`(${S.join(" OR ")})`)}if(d){let E=Array.isArray(d)?d:[d],S=E.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");E.forEach(f=>{T.push(`%${f}%`,`%${f}%`)}),l.push(`(${S.join(" OR ")})`)}let g=l.length>0?`WHERE id IN (${u}) AND ${l.join(" AND ")}`:`WHERE id IN (${u})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),p=Te(e,s.title,s.narrative),m=ge(this.db,p,a);if(m)return{id:m.id,createdAtEpoch:m.created_at_epoch};let T=this.db.prepare(`
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,d,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),m=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let b=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,p,d);u.push(Number(b.lastInsertRowid))}let l=null;if(n){let b=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,d);l=Number(b.lastInsertRowid)}return{observationIds:u,summaryId:l,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,p){let m=p??Date.now(),u=new Date(m).toISOString();return this.db.transaction(()=>{let l=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,u,m);l.push(Number(f.lastInsertRowid))}let b;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(m,o),{observationIds:l,summaryId:b,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),p=[...e],m=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&p.push(o),this.db.prepare(`
    ...
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),p=[...e],m=o?"AND s.project = ?":"";return o&&p.push(o),this.db.prepare(`
      ...
      ${a}
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,p;if(e!==null){let E=`
...
`):null}function kt(r,e,t,s,n,o){let i=[];o?i.push(...Je(r)):i.push(...we(r));let a=null,d="",p=!1;for(let m of e)if(m.type==="summary"){p&&(i.push(""),p=!1,a=null,d="");let u=m.data,T=it(u.displayTime);o?i.push(...et(u,T)):i.push(...Xe(u,T))}else{let u=m.data,T=ct(u.files_modified,n,u.files_read),l=at(u.created_at),g=l!==d,b=g?l:"";d=l;let E=t.has(u.id);if(T!==a&&(p&&i.push(""),o?i.push(...Qe(T)):i.push(...$e(T)),a=T,p=!0),E){let S=xt(u,s);o?i.push(...Ze(u,l,g,S,s)):(p&&!o&&(i.push(""),p=!1),i.push(...Pe(u,b,S,s)),a=null)}else o?i.push(ze(u,l,g,s)):i.push(Fe(u,b,s))}return p&&i.push(""),i}function pt(r,e,t,s,n){let o=[],i=Ut(r);for(let[a,d]of i)o.push(...kt(a,d,e,t,s,n));return o}function _t(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function mt(r,e){let t=[];return e?(t.push(...k("Investigated",r.investigated,c.blue)),t.push(...k("Learned",r.learned,c.yellow)),t.push(...k("Completed",r.completed,c.green)),t.push(...k("Next Steps",r.next_steps,c.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function ut(r,e){return e?tt(r):je(r)}function lt(r,e,t){return!X(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?st(r.totalDiscoveryTokens,r.totalReadTokens):Ge(r.totalDiscoveryTokens,r.totalReadTokens)}var wt={decision:1,discovery:.9,bugfix:.7,feature:.6,refactor:.5,change:.4},$t=.25,Ft=.5,Pt=.25,W=class{static computeScore(e,t,s){let n=wt[e]??.4,o=t<=30?1:Math.max(0,1-(t-30)/150),i=Math.min(1,s/5);return $t*n+Ft*o+Pt*i}static cleanup(e,t,s){let n=performance.now();if(!s.enabled)return{deleted:0,kept:0,orphanedPrompts:0,elapsed_ms:0};let o=Date.now()-s.retentionDays*24*60*60*1e3,i=e.query(`
      SELECT id, type, created_at_epoch, access_count
      FROM observations
      WHERE project = ? AND created_at_epoch < ?
      ORDER BY created_at_epoch DESC
    `).all(t,o);if(i.length===0)return{deleted:0,kept:0,orphanedPrompts:0,elapsed_ms:performance.now()-n};let a=Date.now(),d=i.map(l=>{let g=(a-l.created_at_epoch)/864e5,b=this.computeScore(l.type,g,l.access_count);return{id:l.id,score:b}}),p=[],m=d.filter(l=>l.score<s.scoreThreshold?(p.push(l.id),!1):!0);if(m.length>s.maxKept){m.sort((g,b)=>b.score-g.score);let l=m.splice(s.maxKept);p.push(...l.map(g=>g.id))}let u=0;p.length>0&&e.transaction(()=>{let g=p.map(()=>"?").join(",");e.prepare(`DELETE FROM observations WHERE id IN (${g})`).run(...p),u=e.prepare(`
        ...
        `).run(t).changes})();let T=performance.now()-n;return _.info("RETENTION",`Cleanup complete for ${t}`,{deleted:p.length,kept:m.length,orphanedPrompts:u,elapsed_ms:Math.round(T)}),{deleted:p.length,kept:m.length,orphanedPrompts:u,elapsed_ms:T}}};var Xt=Et.default.join((0,Tt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function jt(r){try{return r?new D(r):new D}catch(e){if(e.code==="ERR_DLOPEN_FAILED"){try{(0,gt.unlinkSync)(Xt)}catch(t){_.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},t)}return _.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw e}}function Gt(r,e){return e?rt(r):He(r)}function Ht(r,e,t,s,n,o,i){let a=[],d=Q(e);a.push(...nt(r,d,s,i));let p=t.slice(0,s.sessionCount),m=Me(p,t),u=te(e,m),T=Le(e,s.fullObservationCount);a.push(...pt(u,T,s,n,i));let l=t[0],g=e[0];_t(s,l,g)&&a.push(...mt(l,i));let b=ee(e,s,o,n);return a.push(...ut(b,i)),a.push(...lt(d,s,i)),a.join(`
`).trimEnd()}async function re(r,e=!1){let t=q(),s=r?.cwd??process.cwd(),n=he(s),o=r?.projects||[n],i=jt(r?.dbPath);if(!i)return"";if(t.retention?.enabled)for(let a of o)try{W.cleanup(i.db,a,t.retention)}catch(d){_.warn("CONTEXT","Retention cleanup failed, continuing",{project:a,error:d instanceof Error?d.message:String(d)})}try{let a=o.length>1?Ae(i,o,t):z(i,n,t),d=o.length>1?Ie(i,o,t):Z(i,n,t);return a.length===0&&d.length===0?Gt(n,e):Ht(n,a,d,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
