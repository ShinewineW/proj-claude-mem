...
- **Layer A**: `observation-filter.ts` — `parseSkipPatterns()`/`shouldSkipObservation()` pre-filter before enqueue. Cached patterns, configurable via `CLAUDE_MEM_SKIP_TOOL_PATTERNS`. Extracts `Bash` commands (`tool_input.command`) with a compound-command guard (`&`, `|`, `;`, `<`, `>`, `` ` ``, `$(` → never skipped, even under `Bash:*`); `layerAStats` module counters (toolExcluded/patternFiltered) exposed via `/api/health` `ai.layerA`.
