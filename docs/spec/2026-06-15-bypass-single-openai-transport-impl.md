...
- **🔒 提交纪律（security/隔离,MUST）：禁止 `git add -A` / `git add .`。** 工作树当前含 6/9 未提交的 Engram 改动（`CONTEXT.md`、`docs/adr/`、`.claude/rules/`、`docs/spec/Manual/...`,用户明确要求**不提交**）+ 本次新增的 ADR 0003 / CONTEXT.md 术语 / 本 impl。每个 commit **只 `git add <该 Task Files 段列出的显式路径>`**,提交前 `git status` 自检无越界。本 impl 的 `git mv` 测试文件也用显式路径。
